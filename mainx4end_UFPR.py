# This is for UFPR Dataset for SR
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hr_train_dataset', type=str, default='UFPRx4')
    #parser.add_argument('--hr_train_dataset', type=str, default='testing')
    parser.add_argument('--HR',default ='./Dataset/UFPRx4'+'/')
    parser.add_argument('--test_dataset', type=str, default='./Dataset/x4')
    ...
    parser.add_argument('--pretrained_sr', default='/UFPR_best.pth', help='sr pretrained base model')

    parser.add_argument('--pretrained', type=bool, default=True)
    parser.add_argument('--save_folder', default='weights/UFPRx44', help='Location to save checkpoint models')
    parser.add_argument('--prefix', default='tpami_residual_filter8', help='Location to save checkpoint models')
    # parser.add_argument('--device', default='0', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--output', default='./Results/UFPRx44/', help='Location to save checkpoint models')

    # parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='x4_phase1.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='models/yolov5x.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/CarNumberTest.yaml', help='data.yaml path')
    ...
    def train(epoch):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):# dbpn train loader
            ...
            ddct = dct.DCT_2D() #고친거버리기
            didct = dct.IDCT_2D()
            # Forward
            prediction=imgs_list[i]
            svim(prediction.float(), './image_check1.jpeg')
            X = ddct(imgs) 
            ...
            X=X/10646.90625
            X=X*255.0
            new1=min(map(min, X[0][0]))
            new2=min(map(min, X[0][1]))
            new3=min(map(min, X[0][2]))

            new4=max(map(max, X[0][0]))
            new5=max(map(max, X[0][1]))
            new6=max(map(max, X[0][2]))
            # #nit8
            print(new1)
            print(new2)
            print(new3)
            ...
            #         Y[0][j]=0
            #         print(Y[i][j])
            # new1=min(map(min, Y[0][0]))
            # new2=min(map(min, Y[0][1])) 
            # new3=min(map(min, Y[0][2]))
            # new7=max(map(max, Y[0][0]))
            # new8=max(map(max, Y[0][1])) 
            # new9=max(map(max, Y[0][2]))
            # print(Y[0][0])
            # Y[0][0]-=new1.item()
            # Y[0][1]-=new2.item()
            # Y[0][2]-=new3.item()
            ...
            # new4=min(map(min, Y[0][0]))
            # new5=min(map(min, Y[0][1])) 
            # new6=min(map(min, Y[0][2]))
            # Y[0][0]/=new7.item()
            # Y[0][1]/=new8.item()
            # Y[0][2]/=new9.item()
            # Y[0][0]*=255.0
            ...
            dct_imgs=imgs+Y
            svim(dct_imgs ,'./image_check5.jpeg')
            with amp.autocast(enabled=cuda):
                # print(imgs_list[i].shape)

                # pred = yolo_model(imgs_list[i])# 이렇게 하면 yolo는 학습됨

          

                pred = yolo_model(imgs)
                
                # svim(pred.float()/255.0, './image_check3.jpeg')
            ...
            # print(sum_yolo_torch.requires_grad)
            # loss/=2.0
        
            epoch_loss += loss.item()
            sum_yolo_loss+=yolo_loss.item()
  
 
            # yolo_loss=yolo_loss
            # print(str(loss)+"\n")
            # print(str(yolo_loss)+"\n")
            # print(yolo_loss.requires_grad)
            # print(yolo_loss.requires_grad)
            # new_loss=yolo_loss#*1.2
            # print(new_loss)
            # new_loss=loss*0.5+yolo_loss
            new_loss=loss+yolo_loss*0.001

            final_loss+=new_loss.item()


            scaler.scale(new_loss).backward()#retain_graph=True)#retain_graph=True)
            ...
            # print(epoch_loss.device())
            # print(epoch_loss.device())
            # print(sum_yolo_torch[0].device())
            # print(sum_yolo_torch[1].item())
            # print(sum_yolo_torch[2].device())
            # print(final_loss.item())
        plot_epoch_loss.append(epoch_loss/len(training_data_loader))
        ...
        plot_final_loss.append(final_loss/len(training_data_loader))
 ...
 # print(results)
        ...
        # f.write(results[4]+result[5]+result[6])
        # f.close()
    ...
    def checkpoint(epoch):
        ...
        save_dir = opt.save_folder
        val_p,val_s,val_l=evalx4.eval(epoch,model_out_path)
        plot_val_sr_loss.append(round(val_l,3))
        plot_val_psnr.append(round(val_p,3))
        plot_val_ssim.append(round(val_s,3))
        f = open(str(save_dir)+"/val_sr_loss.txt",'a')
        f.write(str(plot_val_sr_loss))
        f.close()
        f = open(str(save_dir)+"/val_psnr.txt",'a')
        f.write(str(plot_val_psnr))
        f.close()
        f = open(str(save_dir)+"/val_ssim.txt",'a')
        f.write(str(plot_val_ssim))
        f.close()
        axis = np.linspace(1, epoch, epoch)     
        ...
        plt.grid(True)
        plt.savefig('{}/test_{}.pdf'.format(opt.save_folder, "val_SR_loss"))
        plt.close(fig)
        ...
        plt.ylabel('PSNR')
        plt.grid(True)
        plt.savefig('{}/test_{}.pdf'.format(opt.save_folder, "val_PSNR"))
        plt.close(fig)
        ...
        plt.ylabel('SSIM')
        plt.grid(True)
        plt.savefig('{}/test_{}.pdf'.format(opt.save_folder, "val_SSIM"))
        plt.close(fig)
    ...
    with open(opt.data) as f:
        ...
        if opt.adam:
            ...
            yolo_optimizer = optim.Adam(pg0, lr=1e-4, betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
        else:

            yolo_optimizer = optim.SGD(pg0, lr=1e-4, momentum=hyp['momentum'], nesterov=True)
