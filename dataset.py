...
def augment(img_in, img_tar, img_bic, flip_h=False, rot=False):
...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        apple = self.image_filenames[index].split("\\")
        apple[1]='x'+str(self.upscale_factor)
        _, file = os.path.split(self.image_filenames[index])
        
        apple = "/".join(apple)
        #print(apple)
        input = load_img(apple)
        in_ori=input
        tar_ori=target
        ...
        if self.data_augmentation:
            ...
            in_ori,tar_ori, bi_ori, _ = augment(in_ori, tar_ori, bi_ori)
        
        if self.transform:
            input = self.transform(input)
            bicubic = self.transform(bicubic)
            target = self.transform(target)
            # in_ori = self.transform(in_ori)
            # bi_ori = self.transform(bi_ori)
            # tar_ori = self.transform(tar_ori)
...
class DatasetForCar(data.Dataset):
    def __init__(self, image_dir, patch_size, upscale_factor, data_augmentation, transform=None):
        super(DatasetForCar, self).__init__()
        self.image_filenames = [join(image_dir, x) for x in listdir(image_dir) if is_image_file(x)]
        self.patch_size = patch_size
        self.upscale_factor = upscale_factor
        self.transform = transform
...
class DatasetFromFolderEval(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        bicubic = rescale_img(input, self.upscale_factor)
        # input_patch, target_patch, bicubic_patch, _ = get_patch(input,target,bicubic,self.patch_size, self.upscale_factor)
