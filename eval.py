...
parser.add_argument('--output', default='./Results/SAVE/', help='Location to save checkpoint models')
...
def eval(num=0, newmodelname=''):
    ...
    for batch in testing_data_loader:
        ...
        [one , two] = save_img(prediction.cpu().data, name[0], num)
        aver[0] += one
        aver[1] += two
...
def evalOne():
    ...
    for batch in testing_data_loader:
        ...
        t1 = time.time()
        print("===> Processing: %s || Timer: %.4f sec." % (name[0], (t1 - t0)))
        one, two = save_img(prediction.cpu().data, name[0],opt.model)
    ...
    graphing.graph(lis[0], [], opt.output+'epoch'+opt.model,'PSNR','Vali_pics')     
    graphing.graph(lis[1], [], opt.output+'epoch'+opt.model,'SSIM','Vali_pics')     
    
    print(f"average PSNR is {aver[0]}")
    print(f"average SSIM is {aver[1]}")
    
    f = open(opt.output+"cal.txt", 'a')
    f.write(opt.model+f" : PSNR : {aver[0]}, SSIM : {aver[1]}\n")
    f.close()
